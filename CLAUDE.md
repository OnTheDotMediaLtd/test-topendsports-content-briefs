...
## 🔍 UNIFIED CONTENT VALIDATION (NEW)

### Overview

All generated content MUST pass through the **Unified Content Validator** before delivery.
This integrates validators from `tes-shared-infrastructure`:

| Validator | Purpose | What It Checks |
|-----------|---------|----------------|
| **AI Pattern Detection** | Avoid AI-detectable writing | Colon-heavy headings, formulaic patterns, generic intros |
| **Brand Validation** | Verify sportsbook names | Fake/hallucinated brands, misspellings, unknown names |
| **E-E-A-T Validation** | Google quality signals | Author info, expertise signals, trustworthiness markers |

### Running Validation

```bash
# Validate a single file
python3 scripts/unified_content_validator.py output/example-ai-enhancement.html

# Validate all files in output directory
python3 scripts/unified_content_validator.py --all output/

# Validate markdown file
python3 scripts/unified_content_validator.py output/example-writer-brief.md

# Output JSON report
python3 scripts/unified_content_validator.py --json output/example.html

# Strict mode (fail on warnings too)
python3 scripts/unified_content_validator.py --strict output/example.html

# Custom E-E-A-T threshold (default: 50)
python3 scripts/unified_content_validator.py --eeat-min 60 output/example.html
```
...
# Quick validation
result = validate_content(html_content)
if not result.is_valid:
    print(f"Errors: {result.total_errors}")

# Full validator with options
...
result = validator.validate_file("output/content.html")
result.print_report()
```
...
**AI Pattern Checks:**
- [ ] Less than 30% of headings use colons
- [ ] No formulaic patterns ("Understanding X: A Complete Guide")
- [ ] No generic AI intros ("In today's world of...", "When it comes to...")
- [ ] No repetitive sentence structures

**Brand Validation:**
- [ ] All sportsbook names are verified (FanDuel, BetMGM, DraftKings, etc.)
- [ ] No hallucinated/fake brand names
- [ ] Correct spelling of brand names

**E-E-A-T Requirements:**
- [ ] Author name and credentials present
- [ ] Expert signals (experience, qualifications)
- [ ] Trust signals (sources, responsible gambling notice)
- [ ] Minimum score of 50/100 (configurable)

### Integration with Workflow

Run unified validation after Phase 3 (AI Enhancement):

```
Phase 3 Complete → Run unified_content_validator.py → Delivery
                            ↓
                    If FAILED: Fix issues and re-validate
```

**DO NOT deliver content that fails validation.**
