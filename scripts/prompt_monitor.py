...
class PromptMonitor:
    ...
    def log_usage(
        ...
        status_icon = {"success": "[OK]", "failure": "[FAIL]", "partial": "[WARN]", "skipped": "[SKIP]"}.get(status, "[?]")
        try:
            print(f"[LOGGED] {status_icon} {command} ({entry.category})")
        except UnicodeEncodeError:
            print(f"[LOGGED] {status_icon} {command} ({entry.category})".encode('ascii', errors='replace').decode('ascii'))
