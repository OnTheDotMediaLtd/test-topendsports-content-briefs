# WRITER BRIEF: Best Sports Betting Apps Canada

## ASSIGNED TO: Lewis Humphries

---

## PAGE BASICS

- **URL:** https://www.topendsports.com/sport/betting/canada/betting-apps.htm
- **Target Keyword:** "betting apps canada"
- **Primary Search Volume:** 500/month
- **Total Cluster Volume:** 3,480/month (596% increase)
- **Word Count Target:** 6,500 words
- **Template:** 2 (Comparison/Best-Of)
- **Priority:** HIGH
- **Market Focus:** Canada (Offshore betting apps)

---

## COMPETITOR REFERENCE URLs

Use these Canadian betting sites for competitive analysis and market context:

1. **Covers.com/ca** - https://www.covers.com/betting/canada
   - Use for: Canadian betting market trends, app analysis
2. **ActionNetwork Canada** - https://www.actionnetwork.com/canada
   - Use for: Sports betting app reviews, odds comparison
3. **CanadaSportsBetting** - https://www.canadasportsbetting.ca/online-sportsbook-reviews/mobile-apps/
   - Use for: Mobile app rankings, download guides

**Important:** These are for research context only. DO NOT cite these competitor sites for pros/cons or recommendations. Use TIER 1 sources (App Store, Google Play, Reddit) for brand evaluations.

---

## E-E-A-T AUTHOR REQUIREMENTS

**Author Name:** Lewis Humphries

**Credentials Required:**
- Minimum 3+ years sports betting app testing experience
- Demonstrated expertise in Canadian sports betting market
- Hands-on experience with iOS and Android betting apps
- Knowledge of offshore vs regulated app differences

**Author Bio Link:** [To be added by editorial team]

**E-E-A-T Signals to Include:**
- Author has personally downloaded and tested all 9 featured betting apps
- Direct experience with app-based deposits (Interac, crypto)
- Testing on multiple devices (iOS and Android platforms)
- Knowledge of Canadian sports betting landscape (NHL, CFL focus)

---

## SECONDARY KEYWORD OPTIMIZATION

Map EVERY secondary keyword to a specific section. Use exact titles provided.

| Keyword | Volume | Section Type | Title/Placement |
|---------|--------|--------------|-----------------|
| best sports betting apps canada | 700/mo | H1 | "Best Sports Betting Apps Canada: Complete Guide" |
| best betting apps canada | 500/mo | H2 | "Best Betting Apps Canada: Our Top 9 Picks" |
| sports betting apps canada | 300/mo | Natural | Introduction paragraph (use 2-3 times) |
| gambling apps canada | 300/mo | H2 + H3 | "Types of Gambling Apps Available in Canada" |
| canadian betting apps | 250/mo | Natural | Introduction paragraph |
| sports betting app canada | 200/mo | Natural | Introduction paragraph |
| canadian sports betting apps | 150/mo | Natural | Introduction paragraph |
| top sports betting apps canada | 80/mo | H2 | "Top Sports Betting Apps Canada: Comparison Table" |
| best soccer betting apps canada | 40/mo | H3 | Under "Best Betting Apps by Sport" |
| top betting apps canada | 30/mo | Natural | Introduction paragraph |
| android betting apps canada | 20/mo | H2 | "Android vs iOS Betting Apps: Which is Better?" |
| iphone betting apps canada | 20/mo | H2 | Same section as Android |
| mobile betting apps canada | 20/mo | H2 | "Mobile Betting App Features: What to Look For" |

**VERIFICATION:** Unmapped Keywords: NONE ✓

**Meta Keywords (for AI Enhancement phase):**
betting apps canada, best sports betting apps canada, best betting apps canada, sports betting apps canada, gambling apps canada, canadian betting apps, sports betting app canada, canadian sports betting apps, top sports betting apps canada, best soccer betting apps canada, top betting apps canada, android betting apps canada, iphone betting apps canada, mobile betting apps canada

---

## MANDATORY INTRO FORMAT

**Total:** 100-150 words max

**Structure:**
1. **Opening (40-50 words):** Direct answer with top 3 winners
   - Example: "The best sports betting apps in Canada are Treasure Spins, Royalistplay, and Wyns. These offshore betting apps offer Canadian bettors superior odds, extensive NHL and CFL coverage, and fast crypto payouts."

**FORBIDDEN:**
- ❌ "Welcome to..."
- ❌ "Looking for..."
- ❌ "In this guide..."
- ❌ Rhetorical questions
- ❌ Over 150 words
- ❌ NO affiliate disclosure in content (it's in website sidebar)

---

## NO DATED LANGUAGE IN H1/TITLE

❌ **NEVER USE:** "October 2025", "Review 2025", "Best Apps 2025"

✅ **USE INSTEAD:**
- H1: "Best Sports Betting Apps Canada: Complete Guide"
- Add "Last Updated" badge immediately after H1 (AI will implement)

**Where dates ARE okay:**
- Last Updated badge
- Within content: "As of December 2025..."
- T&Cs verification date

---

## WORD COUNT BY SECTION

| Section | Target Words |
|---------|--------------|
| Introduction | 100-150 |
| Comparison Table | 400-500 |
| Brand Reviews (9 total) | 3,550 |
| Payment Methods | 350-400 |
| Best Bonuses | 400-450 |
| How to Download | 400-450 |
| Best Apps by Sport | 500-550 |
| Android vs iOS | 350-400 |
| Mobile Features | 300-350 |
| Types of Gambling Apps | 250-300 |
| FAQs (10 total) | 1,200-1,400 |
| Responsible Gambling | 150-200 |
| **TOTAL** | **~6,500** |

---

## V2 CRITICAL REQUIREMENTS

**This brief follows V2 standards. Ensure ALL requirements are met:**

1. ✅ **Keyword Mapping:** Every keyword mapped to specific section (verified above)
2. ✅ **Mobile Experience:** 100-150 words per brand (MANDATORY for all 9 brands)
3. ✅ **Payment Methods:** Dedicated 350-400w section with comparison table
4. ✅ **Calculator Links:** 1-2 contextual links to parlay/odds calculators
5. ✅ **Intro Format:** 100-150w max, NO "Welcome to...", NO affiliate disclosure
6. ✅ **Source Requirements:** TIER 1 (App Store, Reddit), TIER 2 (Brand sites), TIER 4 warning (No affiliate sources for pros/cons)
7. ✅ **Compliance:** Canada-specific helplines, 19+ age, NO affiliate disclosure in content

---

## CONTENT OUTLINE

### H1: Best Sports Betting Apps Canada: Complete Guide
*[Last Updated badge will be added in Phase 3]*

### Introduction (100-150 words)
- Direct answer with Treasure Spins, Royalistplay, Wyns as top 3
- Mention offshore betting focus
- **Natural keyword usage:** sports betting apps canada, canadian betting apps, sports betting app canada, canadian sports betting apps, top betting apps canada

---

### H2: Top Sports Betting Apps Canada: Comparison Table (400-500 words)

**Writer must cover:**
- Comprehensive comparison table with all 9 brands and key metrics
- App Store ratings (iOS) and Google Play ratings (Android) for each brand
- Welcome bonuses, NHL markets availability, and live betting features
- Payment methods (Interac, crypto) with clear indicators
- Download links for both platforms (iOS and Android)

Create comprehensive comparison table with all 9 brands:

**Columns to include:**
- Brand Name + Logo
- App Store Rating (iOS)
- Google Play Rating (Android)
- Welcome Bonus
- NHL Markets (Yes/No or # of markets)
- Live Betting (Yes/No)
- Interac E-Transfer (Yes/No)
- Crypto Payments (BTC, ETH, LTC)
- Download Links (App Store + Google Play)

**Notes:**
- Interactive table (Phase 3 will add filtering)
- Sort by: Rating, Bonus Amount, Payment Methods
- Phase 3 handles responsive design

---

### H2: [Brand #1] Treasure Spins Betting App (500-550 words)

#### H3: Key Features (150-200 words)
- Signature features
- Betting markets (NHL, CFL emphasis)
- Odds competitiveness
- Live betting capabilities
- Unique offerings

#### H3: Mobile Experience (100-150 words) **[CRITICAL - MANDATORY]**
**MUST INCLUDE:**
- App Store rating: "4.X/5 stars (X,XXX reviews) - verified [date]"
- Google Play rating: "4.X/5 stars (X,XXX reviews) - verified [date]"
- Navigation speed and responsiveness
- Live betting experience on mobile
- Unique mobile features (Touch ID, Face ID, notifications)
- Performance notes (crashes, loading times)

**Source Requirements:**
- MUST cite App Store for iOS rating
- MUST cite Google Play for Android rating
- Reddit user feedback if available

#### H3: Pros & Cons (100-150 words)
- 3-5 pros with brief explanations
- 3-5 cons with brief explanations
- Focus on app-specific pros/cons

#### H3: Current Bonus (75-100 words)
- Exact bonus amount
- Wagering requirements
- Time limits
- Restrictions

---

### H2: [Brand #2] Royalistplay Betting App (450-500 words)

*[Same subsection structure as Brand #1: Key Features, Mobile Experience, Pros & Cons, Current Bonus]*

---

### H2: [Brand #3] Wyns Betting App (400-450 words)

*[Same subsection structure: Key Features, Mobile Experience, Pros & Cons, Current Bonus]*

---

### H2: [Brand #4] Lucky7even Betting App (350-400 words)

*[Same subsection structure: Key Features, Mobile Experience, Pros & Cons, Current Bonus]*

---

### H2: [Brand #5] Festival Play Betting App (350-400 words)

*[Same subsection structure: Key Features, Mobile Experience, Pros & Cons, Current Bonus]*

---

### H2: [Brand #6] Lizaro Betting App (350-400 words)

*[Same subsection structure: Key Features, Mobile Experience, Pros & Cons, Current Bonus]*

---

### H2: [Brand #7] Legendplay Betting App (350-400 words)

*[Same subsection structure: Key Features, Mobile Experience, Pros & Cons, Current Bonus]*

---

### H2: [Brand #8] Funbet Betting App (350-400 words)

*[Same subsection structure: Key Features, Mobile Experience, Pros & Cons, Current Bonus]*

---

### H2: [Brand #9] DirectionBet Betting App (350-400 words)

*[Same subsection structure: Key Features, Mobile Experience, Pros & Cons, Current Bonus]*

---

### H2: Popular Payment Methods on Canadian Betting Apps (350-400 words)

**Writer must cover:**
- Interac e-Transfer as #1 payment method for Canadian app users
- Cryptocurrency options (Bitcoin, Ethereum, Litecoin) with processing times
- Traditional methods (credit/debit) and Canadian bank restrictions
- Which apps accept which payment methods (comparison context)

#### H3: Interac E-Transfer (100-125 words)
- Most popular Canadian method
- Processing times (instant deposits, 24-48hr withdrawals)
- Limits and fees
- Which apps accept Interac

#### H3: Cryptocurrency Options (100-125 words)
- Bitcoin (BTC)
- Ethereum (ETH)
- Litecoin (LTC)
- Advantages: Anonymity, speed, no bank involvement
- Processing times (10 min - 2 hours)
- Best apps for crypto

#### H3: Traditional Methods (Credit/Debit) (75-100 words)
- Visa/Mastercard
- Limitations (some Canadian banks block gambling)
- Processing times
- Best for whom

---

### H2: Best Bonuses Across Canadian Betting Apps (400-450 words)

**Competitor Gap:** Top-ranking competitor has this section - we need it too.

**Writer must cover:**
- Welcome bonus comparison across all 9 apps with wagering requirements
- App-exclusive bonus offers (mobile-only promotions, download bonuses)
- Ongoing promotions for mobile users (reload bonuses, parlay boosts)
- Bonus comparison table with amounts, wagering, and app-exclusive offers

#### H3: Welcome Bonus Comparison (150-175 words)
- Compare welcome bonuses across all 9 apps
- Bonus amounts (match percentage, max bonus)
- Wagering requirements (20x, 30x, etc.)
- Time limits to meet requirements
- Best overall value (top 3 picks)

#### H3: App-Exclusive Bonus Offers (125-150 words)
- Mobile-only promotions
- Download bonuses (bonus for installing app)
- Push notification exclusive offers
- In-app promo code entry
- Which apps offer the best mobile-exclusive deals

#### H3: Ongoing Promotions for Mobile Users (100-125 words)
- Reload bonuses
- Parlay boosts (especially for NHL/CFL)
- Free bet promotions
- Loyalty programs accessible via app
- Best apps for regular promotions (top 3)

**Table Required:**
| App | Welcome Bonus | Wagering | App-Exclusive Offer |
|-----|---------------|----------|---------------------|
| Treasure Spins | [Amount] | [Req] | [Offer] |
| Royalistplay | [Amount] | [Req] | [Offer] |
| ... (all 9 brands) | | | |

---

### H2: How to Download Betting Apps in Canada (400-450 words)

**Writer must cover:**
- iOS download process (App Store, web apps, direct links from brand sites)
- Android download process (Google Play, APK downloads for offshore apps)
- Security settings for Android (allow unknown sources for APKs)
- Step-by-step installation and account creation for both platforms

#### H3: iOS Download Guide (150-200 words)
- App Store search method
- Direct download links from brand sites
- Web app alternatives (if App Store restricted)
- Installation steps
- Account creation

#### H3: Android Download Guide (150-200 words)
- Google Play availability
- APK downloads (offshore apps)
- Security settings (allow unknown sources)
- Installation steps
- Account verification

**Internal Link:** Link to "mobile betting guide"

---

### H2: Best Betting Apps by Sport (500-550 words)

**Writer must cover:**
- Best NHL betting apps (top 3) with puck line markets and player props
- Best CFL betting apps with Grey Cup coverage (uniquely Canadian)
- Best soccer betting apps for EPL, Champions League, and CPL
- Best NBA/NFL betting apps with live betting and player props

#### H3: Best NHL Betting Apps (150-175 words)
**Emphasis on Canadian market - NHL is #1 sport**

- Top 3 apps for NHL betting
- Puck line markets
- Player props (goals, assists, saves)
- Live betting during games
- Canadian team coverage (Maple Leafs, Oilers, Canadiens, etc.)

**Internal Link:** Link to "NHL betting in Canada"

#### H3: Best CFL Betting Apps (125-150 words)
**Uniquely Canadian sport**

- Top 3 apps for CFL
- CFL coverage depth
- Grey Cup markets
- Live betting availability
- Which offshore apps cover CFL best

#### H3: Best Soccer Betting Apps Canada (125-150 words)
**Target Keyword:** best soccer betting apps canada (40/mo)

- Top 3 apps for soccer
- Premier League, Champions League, MLS coverage
- Canadian Premier League (CPL)
- In-play soccer betting

**Internal Link:** Link to "soccer betting apps Canada"

#### H3: Best NBA/NFL Betting Apps (100-125 words)
- Top apps for NBA and NFL
- Player props and parlays
- Live betting during games

**Internal Link:** Link to "NFL betting apps"

---

### H2: Android vs iOS Betting Apps: Which is Better? (350-400 words)

**Target Keywords:** android betting apps canada (20/mo), iphone betting apps canada (20/mo)

**Writer must cover:**
- App Store vs Google Play availability for offshore betting apps
- Performance comparison (speed, crashes, stability) between platforms
- Feature parity analysis (do Android and iOS have same features?)
- Download process differences (APK for Android, web apps for iOS restrictions)

**Compare:**
- App Store vs Google Play availability
- Performance comparison (speed, crashes)
- Feature parity (do Android and iOS have same features?)
- Download process differences
- Recommendation by platform

**Cover:**
- iOS: Cleaner interface, more stable, stricter App Store rules
- Android: More flexibility, APK downloads, wider device compatibility
- Performance tests (if available)

---

### H2: Mobile Betting App Features: What to Look For (300-350 words)

**Target Keyword:** mobile betting apps canada (20/mo)

**Writer must cover:**
- Essential features (live betting speed, Touch ID/Face ID, push notifications, cash out)
- Bonus features (dark mode, customizable home screen, stats integration)
- Contextual links to parlay calculator and odds calculator tools
- Which apps excel at specific features (comparison context)

**Essential Features:**
- Live betting speed (real-time odds updates)
- Touch ID / Face ID (biometric login)
- Push notifications (bet results, promotions)
- Bet slip functionality (quick bet placement)
- Cash out options (early payout)
- Live streaming (if available)
- Parlay builder interface
- Deposit/withdrawal from app

**Bonus Features:**
- Dark mode
- Customizable home screen
- Stats integration
- Bet tracking and history

**REQUIRED: Calculator Tool Links (1-2 contextual links)**
- Link to "parlay calculator" where discussing parlay builders
- Link to "odds calculator" where discussing odds comparison

---

### H2: Types of Gambling Apps Available in Canada (250-300 words)

**Target Keyword:** gambling apps canada (300/mo)

**Writer must cover:**
- Sports betting apps (focus on sports markets, live betting, best examples)
- Casino apps (slots, table games, live dealer options)
- Hybrid apps (sports + casino all-in-one platforms with shared wallets)

#### H3: Sports Betting Apps (75-100 words)
- Focus on sports markets
- Live betting and props
- Best for sports bettors
- Examples: Treasure Spins, Royalistplay

#### H3: Casino Apps (75-100 words)
- Slots, table games, live dealer
- RNG vs live casino
- Best for casino players
- Examples: (mention if any brands offer casino)

#### H3: Hybrid Apps (Sports + Casino) (75-100 words)
- All-in-one platforms
- Shared wallets
- Best for versatility
- Examples: Most offshore apps offer both

---

### H2: Frequently Asked Questions (1,200-1,400 words)

**10 FAQs, 120-150 words each. Target high-value keywords.**

#### 1. What are the best sports betting apps in Canada?
**Target:** best sports betting apps canada (700/mo)
- Top 3 picks: Treasure Spins, Royalistplay, Wyns
- Why they rank #1-3
- Key features of each
- Offshore vs Ontario apps difference

#### 2. Are betting apps legal in Canada?
**Target:** betting apps legal canada
- Provincial regulations overview
- Ontario's regulated market
- Offshore apps legality (gray area)
- Age requirements (19+ in most provinces, 18+ in AB, MB, QC)
- Safe to use offshore apps? (discuss risks)

#### 3. Which betting app is best for NHL betting?
**Target:** nhl betting apps canada
- Top 3 apps for NHL
- Puck line and player prop availability
- Live betting speed during games
- Canadian team coverage (Maple Leafs, Oilers, Canadiens)

#### 4. Can I use Android betting apps in Canada?
**Target:** android betting apps canada (20/mo)
- Android app availability
- Download process (APK vs Google Play)
- Performance comparison to iOS
- Best Android betting apps (top 3)

#### 5. Do Canadian betting apps accept Interac?
**Target:** interac betting apps
- Interac E-Transfer availability
- Processing times (instant deposits, 24-48hr withdrawals)
- Limits and fees
- Best apps for Interac (list 5+)

#### 6. Can I bet on CFL games with Canadian betting apps?
**Target:** cfl betting apps
- CFL coverage by offshore apps
- Best apps for CFL betting (top 3)
- Grey Cup markets
- Live betting availability during CFL games

#### 7. Do betting apps in Canada accept cryptocurrency?
**Target:** crypto betting apps canada
- Crypto payment options (Bitcoin, Ethereum, Litecoin)
- Advantages of crypto betting (anonymity, speed)
- Processing times (10 min - 2 hours)
- Best apps for crypto (list top 5)

#### 8. How do I download betting apps on iPhone in Canada?
**Target:** iphone betting apps canada (20/mo)
- App Store availability (limited for offshore)
- Geographic restrictions
- Web app alternatives (how to add to home screen)
- Step-by-step download guide

#### 9. Which betting app has the best mobile experience?
**Target:** mobile betting apps canada (20/mo)
- Top 3 for mobile UX
- Key features (live betting, cash out, notifications)
- App Store ratings (iOS and Android)
- Performance and speed comparison

#### 10. Which betting apps have the best welcome bonuses?
**Target:** betting app bonuses canada
- Top 3 apps for welcome bonus value
- Bonus amounts and wagering requirements
- App-exclusive bonus offers
- How to claim bonuses via mobile app

---

### H2: Responsible Gambling in Canada (150-200 words)

**Required Compliance Section**

**Include:**
- Age requirements: 19+ (18+ in Alberta, Manitoba, Quebec)
- ConnexOntario: 1-866-531-2600
- Self-exclusion programs
- Deposit limits and responsible gambling tools
- Warning signs of problem gambling
- Resources:
  - Responsible Gambling Council: https://www.responsiblegambling.org/
  - Canadian Centre on Substance Use and Addiction: https://www.ccsa.ca/
  - Gamblers Anonymous Canada
  - ConnexOntario Helpline: 1-866-531-2600 (Ontario problem gambling support)
  - Provincial helplines:
    - Alberta: 1-866-332-2322
    - British Columbia: 1-888-795-6111
    - Quebec: 1-800-461-0140
    - Other provinces: Contact local health services

**Offshore Disclaimer:**
"Note that offshore betting sites operate outside Canadian jurisdiction. While popular among Canadian bettors, they are not regulated by Canadian gaming authorities."

**NO affiliate disclosure in content (it's in website sidebar)**

---

## SOURCE REQUIREMENTS

### TIER 1 (Primary) - MUST USE:

1. **App Store Reviews**
   - Citation: "App Store rating: 4.X/5 stars (X,XXX reviews) - verified December 2025"
   - Use for: Mobile experience ratings (iOS)
   - **MANDATORY for each brand's Mobile Experience section**

2. **Google Play Store Reviews**
   - Citation: "Google Play rating: 4.X/5 stars (X,XXX reviews) - verified December 2025"
   - Use for: Mobile experience ratings (Android)
   - **MANDATORY for each brand's Mobile Experience section**

3. **Reddit: r/sportsbook**
   - Citation: "Reddit users in r/sportsbook report [finding] (analyzed X threads)"
   - Use for: User experiences, pros/cons, app performance

4. **Reddit: r/AskCanada, r/canada**
   - Citation: "Canadian users on Reddit note [finding]"
   - Use for: Canadian-specific betting experiences

### TIER 2 (Verification):
- Brand official websites (for features, bonuses, T&Cs only)
- AGCO (for Ontario regulatory context)

### TIER 4 (Sparingly):
- Affiliate sites for research gaps ONLY
- ❌ NEVER cite affiliate sites for pros/cons

---

## BRANDS TO FEATURE

All 9 brands must include 4 subsections each:

1. **Treasure Spins** (Position #1)
   - WHY: Top offshore brand for Canadian market
   - EMPHASIS: Strong mobile app, extensive NHL coverage
   - WORD COUNT: 500-550 words

2. **Royalistplay** (Position #2)
   - WHY: Premium offshore sportsbook
   - EMPHASIS: Excellent odds, crypto-friendly
   - WORD COUNT: 450-500 words

3. **Wyns** (Position #3)
   - WHY: Strong mobile app experience
   - EMPHASIS: User-friendly interface, fast payouts
   - WORD COUNT: 400-450 words

4. **Lucky7even** (Position #4)
   - WHY: Competitive odds and bonuses
   - EMPHASIS: Generous promotions, live betting
   - WORD COUNT: 350-400 words

5. **Festival Play** (Position #5)
   - WHY: Growing Canadian market presence
   - EMPHASIS: CFL coverage, Canadian payment methods
   - WORD COUNT: 350-400 words

6. **Lizaro** (Position #6)
   - WHY: Excellent live betting features
   - EMPHASIS: In-play odds updates, mobile live betting
   - WORD COUNT: 350-400 words

7. **Legendplay** (Position #7)
   - WHY: Wide sports coverage
   - EMPHASIS: International sports, soccer betting
   - WORD COUNT: 350-400 words

8. **Funbet** (Position #8)
   - WHY: User-friendly mobile interface
   - EMPHASIS: Simple navigation, beginner-friendly
   - WORD COUNT: 350-400 words

9. **DirectionBet** (Position #9)
   - WHY: Fast payouts and cryptocurrency support
   - EMPHASIS: Crypto payments (BTC, ETH, LTC), quick withdrawals
   - WORD COUNT: 350-400 words

---

## INTERNAL LINKS (8 MINIMUM)

**MUST include these internal links (verified to exist in site structure):**

1. **"Canada sports betting hub"** → /sport/betting/canada/index.htm
   - Placement: Introduction

2. **"parlay calculator"** → /sport/betting-tools/parlay-calculator.htm
   - Placement: Mobile features section, Parlay builder mentions

3. **"Best NHL betting sites"** → /sport/betting/nhl/index.htm
   - Placement: Best NHL betting apps section

4. **"Best NFL betting sites"** → /sport/betting/nfl/index.htm
   - Placement: Best apps by sport section (NFL mention)

5. **"Best soccer betting sites"** → /sport/betting/soccer/index.htm
   - Placement: Best soccer betting apps section

6. **"New betting sites Canada"** → /sport/betting/canada/new-betting-sites.htm
   - Placement: Introduction or brand comparison

7. **"NFL parlays guide"** → /sport/betting/nfl/parlays.htm
   - Placement: Parlay/accumulator mentions

8. **"Same game parlay guide"** → /sport/betting/guides/same-game-parlay.htm
   - Placement: Parlay builder or SGP mentions

**Note:** Only link to pages that exist in the site structure. Do not create placeholder links.

---

## COMPLIANCE CHECKLIST

Every page MUST include:

- [ ] Age requirement: 19+ (18+ in Alberta, Manitoba, Quebec)
- [ ] Responsible gambling resources (Responsible Gambling Council, provincial helplines)
- [ ] Responsible gambling section (150-200 words at bottom)
- [ ] Offshore disclaimer: "Note that offshore betting sites operate outside Canadian jurisdiction"

**Forbidden language:**
- ❌ "Guaranteed wins"
- ❌ "Can't lose"
- ❌ "Risk-free"
- ❌ "Beat the house"

---

## UNIQUE ANGLES FOR THIS PAGE

1. **Offshore betting focus** - Emphasize non-Ontario market (most of Canada)
2. **Canadian payment methods** - Interac E-Transfer prominence
3. **Hockey betting** - NHL as primary sport focus
4. **Provincial regulations** - Age differences by province
5. **Calculator integration** - Link to parlay and odds calculators
6. **Cryptocurrency** - Offshore books accept crypto (BTC, ETH, LTC)
7. **CFL coverage** - Uniquely Canadian sport
8. **Mobile-first approach** - Every brand gets dedicated Mobile Experience section (100-150 words)
9. **App Store ratings** - MANDATORY iOS and Android ratings for each brand

---

## CRITICAL REQUIREMENTS

### Mobile Experience Section (100-150 words PER BRAND)
**This is CRITICAL and MANDATORY for every brand.**

Each brand's Mobile Experience section MUST include:
1. App Store rating with citation
2. Google Play rating with citation
3. Navigation and performance notes
4. Live betting experience on mobile
5. Unique mobile features

### App Store Ratings Required
- iOS rating from App Store
- Android rating from Google Play Store
- Verification date (December 2025)

### Canadian Sports Focus
- NHL (primary focus)
- CFL (uniquely Canadian)
- Soccer (growing popularity)
- NBA/NFL (also popular)

---

## COMPETITOR ANALYSIS

**#1 Ranking Page:** https://www.canadasportsbetting.ca/online-sportsbook-reviews/mobile-apps/

**Their Strengths:**
- Comprehensive comparison table
- Per-brand download links
- Bonus code integration

**Our Advantages (content gaps to fill):**
- 10 FAQs (they have 6)
- Payment methods comparison section
- Calculator tool integration
- Sport-specific app rankings (NHL, CFL, NFL, NBA, Soccer)
- Offshore vs Ontario regulatory discussion
- Deeper mobile experience analysis (dedicated 100-150w per brand)

**Target Word Count:** 6,500 words (vs their 5,735 = 13% longer)

---

## FINAL DELIVERABLE CHECKLIST (V3 STANDARDS)

Before submitting to AI Enhancement (Phase 3):

- [ ] 6,500 words total
- [ ] All 13 secondary keywords mapped to sections with "Unmapped Keywords: NONE" verified
- [ ] **Keyword Volume Total:** 3,480/month (596% increase over primary keyword) ⭐ **V3 REQUIRED**
- [ ] 10 FAQs targeting keywords (120-150 words each)
- [ ] Mobile Experience section for ALL 9 brands (100-150 words each) ⭐ **V2 CRITICAL**
- [ ] App Store ratings cited for each brand (iOS + Android)
- [ ] **"Writer must cover" bullets** under each H2 section (3-5 specific points) ⭐ **V3 REQUIRED**
- [ ] **Competitor Reference URLs section** included (Covers.com/ca, ActionNetwork, CanadaSportsBetting) ⭐ **V3 REQUIRED**
- [ ] **E-E-A-T Author Requirements section** included (credentials, bio link, experience signals) ⭐ **V3 REQUIRED**
- [ ] **Word Count by Section Table** included ⭐ **V3 REQUIRED**
- [ ] Payment Methods Comparison section (350-400w with table) ⭐ **V2 CRITICAL**
- [ ] Calculator Tool Links (1-2 contextual links to parlay/odds calculators) ⭐ **V2 CRITICAL**
- [ ] 8 internal links included (verified existing pages only)
- [ ] Responsible gambling section (150-200 words) with specific Canadian helplines
- [ ] Offshore disclaimer included
- [ ] NO dated language in H1/title
- [ ] NO affiliate disclosure in content (it's in sidebar) ⭐ **V2 CRITICAL**
- [ ] All sources are TIER 1 (App Store, Google Play, Reddit)
- [ ] NHL and CFL betting prominently featured
- [ ] Interac and crypto payment methods covered
- [ ] App bonuses comparison section included

---

**Questions?** Contact project manager before starting.

**Deadline:** [To be determined]

**Next Phase:** AI Enhancement (Phase 3) will add interactive comparison table, schema markup, and technical implementation.
