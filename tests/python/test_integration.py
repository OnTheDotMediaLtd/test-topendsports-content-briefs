...
# Add scripts and tests/python to path
...
class TestAutomationOrchestration:
    ...
    def test_run_command_success(self, automation_env):
        ...
        # Run a simple successful command (platform-aware)
        if sys.platform == "win32":
            code, stdout, stderr = runner.run_command(["cmd", "/c", "echo", "test"])
        else:
    ...
    def test_run_command_failure(self, automation_env):
        ...
        # Run a command that should fail (platform-aware)
        if sys.platform == "win32":
            code, stdout, stderr = runner.run_command(["cmd", "/c", "exit", "1"])
        else:
...
class TestErrorRecoveryAndCleanup:
    ...
    def test_missing_lessons_file(self, cleanup_system, monkeypatch):
        ...
        # The system creates the directory and file if missing
        # Verify either the file was created or lessons were generated successfully
        assert fake_lessons.exists() or len(lessons) > 0
